...
router.put('/update/:id', async (req, res) => {
  try {
    const user = getUserForRequest(req);
    const shop = await Shop.findOne({ _id: req.params.id, userProfile: user }).lean();
    ...
    shop.incorporationType = req.body.incorporationType || shop.incorporationType;
...
router.delete('/delete', async (req, res) => {
  try {
    ...
    const shops = await Shop.find({ _id: { $in: ids }, userProfile: user }).lean();
