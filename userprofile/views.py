...
def SignUp(request):
    ...
    user = User.objects.create_user(email=data[EMAIL], password=data[PASSWORD], pan_number=data[PAN], mobile=data[MOBILE], name=data[NAME], country=data[COUNTRY], incorporation_type = data[INCORPORATION], fssai_number= data[FSSAI], bank_account=data[BANK], account_number=data[ACCOUNT], ifsc_code=data[IFSC], gst_number= data[GST], legal_entity_name=data[LEGAL])
    
    return Response({'status': 200, 'data': data, 'message': user.id})
